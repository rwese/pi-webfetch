...
/**
 * Factory for creating default providers
 */
export interface ProviderFactory {
	createDefaultProvider(): WebfetchProvider;
	createClawfetchProvider(): WebfetchProvider;
	createGhCliProvider(): WebfetchProvider;
}
...
export const defaultProviderFactory: ProviderFactory = {
	createDefaultProvider: () => new DefaultProvider(),
	createClawfetchProvider: () => new ClawfetchProvider(),
	createGhCliProvider: () => new GhCliProvider(),
};
...
export class ProviderManager {
	 ...
	 * @param factory - Factory for creating providers (for DI/testing)
	 * @param providers - Pre-registered providers (for DI/testing)
	 */
	constructor(
		...
		// Register pre-injected providers first (allows overrides)
		if (providers) {
		...
		// Register default providers if none were injected
	...
	private registerDefaultProviders(): void {
		...
		const defaultProviders: WebfetchProvider[] = [
			this.factory.createDefaultProvider(),
			this.factory.createClawfetchProvider(),
			this.factory.createGhCliProvider(),
		];
	 ...
	 */
	async fetch(url: string, config?: FetchConfig): Promise<WebfetchResult> {
